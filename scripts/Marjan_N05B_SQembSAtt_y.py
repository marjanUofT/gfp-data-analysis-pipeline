...
# Add the directory containing your_script.py to the Python path
# project_path = r"C:\Users\Marjan\OneDrive - University of Toronto\01 - UofT\Github\seq-to-func\N_DataProcessing"
...
print("Hiiiiiiiiiiiiiiiiii N05B")
#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$#
...
#                         "Marjan_N03_" + dataset_nme + "_embedding_Unirep.p"      ,      # 13
#                         ]
...
# print(f"RESULT FOOOOOlder: {results_folder}")
#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$#
...
#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$# 
def preparationInput(results_folder, embedding_file, Step_code, dataset_nme, prpty_select, NN_type, log_value, screen_bool):
    ...
    #====================================================================================================#
    ## Test code Marjan: to change the working dir to seq-to-func folder 
    # Check the current working directory
    # current_directory = os.getcwd()
    # print("Current Directory:", current_directory)

    # # Change the working directory
    # new_directory = r'C:\Users\Marjan\OneDrive - University of Toronto\01 - UofT\Github\seq-to-func'
    # os.chdir(new_directory)

    # # Verify the change
    # print("New Current Directory:", os.getcwd())
    ## End Test code Marjan
    #====================================================================================================#
    ...
    return results_folder, embedding_code, results_sub_folder
...
#             f.flush()
#--------------------------------------------------#
def printVariables(results_sub_folder, dataset_nme, embedding_file, log_value, screen_bool, NN_type, seed, epoch_num, batch_size, learning_rate, hyperparameters_dict):
    # orig_stdout = sys.stdout
    # f = open(results_sub_folder / 'print_out.txt', 'w')
    # sys.stdout = Tee(sys.stdout, f)
    #--------------------------------------------------#
    print(">>> Initializing hyperparameters and settings... ")
    print("="*80)
    #--------------------------------------------------#
...
def Get_X_y_data(X_seqs_all_hiddens_list, properties_dict, prpty_select, log_value):
    ...
    #Marjan: to track seq
    seq_data = properties_dict['sequence']
    ...
    return X_seqs_all_hiddens, y_seqs_prpty, aa_seqs

###################################################################################################################
###################################################################################################################
def getAllHiddens(NN_type, X_seqs_all_hiddens_list, properties_dict, log_value, prpty_select): 
    # Currently doesnt support classfication model.
    if NN_type=="Reg":
        X_seqs_all_hiddens, y_seqs_prpty, aa_seqs = Get_X_y_data(X_seqs_all_hiddens_list, properties_dict, prpty_select, log_value)
    # elif NN_type=="Clf":
    #     X_seqs_all_hiddens, y_seqs_prpty = Get_X_y_data_clf(X_seqs_all_hiddens_list, properties_dict, prpty_select, log_value)
...
###################################################################################################################
#====================================================================================================#
def printInput(X_seqs_all_hiddens_list, y_seqs_prpty):
    ...
    print("\nNumber of Data Points (#y-values): ", count_y)
    return X_seqs_all_hiddens_dim, X_seqs_num, seqs_max_len, NN_input_dim, count_y
...
###################################################################################################################
def split_data(X_seqs_all_hiddens, y_seqs_prpty, train_split, test_split, random_state = seed, log_value=False):
    #y_seqs_prpty=np.log10(y_seqs_prpty)
    ...
    return X_tr, y_tr, X_ts, y_ts, X_va, y_va, y_scalar
...
class ATT_dataset(data.Dataset):
    def __init__(self, embedding, label, seqs, X_scaler = None):
        super().__init__()
    ...
    def __getitem__(self, idx):
        return self.embedding[idx], self.label[idx], self.seqs[idx]
    def collate_fn(self, batch:List[Tuple[Any, ...]]) -> Dict[str, torch.Tensor]:
        embedding, target, seqs = zip(*batch)
        ...
        #sequences = [seq for seq in embedding]  # Marjan: Capture original sequences
        ...
        #Marjan: add sequences to output
        'sequences': list(seqs)}

#====================================================================================================#
def ATT_loader(dataset_class, seqs,
               X_tr_seqs, y_tr,
               X_va_seqs, y_va,
               X_ts_seqs, y_ts,
               batch_size,):
    X_y_tr = dataset_class(list(X_tr_seqs), y_tr, seqs, X_scaler = None)
    X_y_va = dataset_class(list(X_va_seqs), y_va, seqs, X_y_tr.X_scaler)
    X_y_ts = dataset_class(list(X_ts_seqs), y_ts, seqs, X_y_tr.X_scaler)
...
#############################################  Marjan: Loader with no y values#####################################
class ATT_dataset_no_y_values(data.Dataset):
    def __init__(self, embedding, seqs, X_scaler=None):
        super().__init__()
        self.embedding = embedding
        self.seqs = seqs
        self.X_scaler = X_scaler
    ...
    def __getitem__(self, idx):
        return self.embedding[idx], self.seqs[idx]

    def collate_fn(self, batch: List[Tuple[Any, ...]]) -> Dict[str, torch.Tensor]:
        embedding, seqs = zip(*batch)
        batch_size = len(embedding)
        emb_dim = embedding[0].shape[1]
        max_len = np.max([s.shape[0] for s in embedding], 0)
        arra = np.full([batch_size, max_len, emb_dim], 0.0)
        ...
        arra = np.reshape(arra, (batch_size, max_len * emb_dim))
        #self.X_scaler = StandardScaler() if self.X_scaler is None else self.X_scaler
        self.X_scaler = sklearn.preprocessing.StandardScaler() if self.X_scaler is None else self.X_scaler
        arra = self.X_scaler.transform(arra) if hasattr(self.X_scaler, "n_features_in_") else self.X_scaler.fit_transform(arra)
        arra = np.reshape(arra, (batch_size, max_len, emb_dim))
        return {
            'embedding': torch.from_numpy(arra),
            'mask': torch.from_numpy(seq_mask),
            'sequences': list(seqs)
        }

def ATT_loader_no_y_values(dataset_class, seqs, embeddings, batch_size):
    dataset = dataset_class(list(embeddings), seqs, X_scaler=None)
    loader = data.DataLoader(dataset, batch_size, False, collate_fn=dataset.collate_fn)
    return loader
...
class PoswiseFeedForwardNet(nn.Module):
    def __init__(self, d_model, d_ff):
        ...
        # # marjan: Add sigmoid activation functio to force and learn model to do the prediction [0,1] only
    ...
    def forward(self, inputs, input_emb):
        ...
        # # marjan: Apply sigmoid activation to constrain output to [0, 1]
...
###################################################################################################################
def loadddingModel(NN_input_dim, d_k, n_heads, d_v, last_hid, learning_rate):
    '''
...
def train(
    ...
    for epoch in range(epoch_num): 
        ...
        for one_seq_ppt_group in train_loader:
            ...
            # #marjan: customize the loss to force model to learn do the prediction between [0,1], cus Sarkysian has [0,1] values
            # standard_loss = criterion(output, target.view(-1,1))
            # # Penalty for out-of-range values
            # penalty = torch.sum((output < 0) | (output > 1)).float()
            # loss = standard_loss + penalty
        ...
        #Marjan: Save the model state_dict
        epoch_time = datetime.now().strftime("%Y%m%d_%H%M%S")
        epoch_model_file = model_epoch_folder / f"epoch_{epoch+1}_{epoch_time}.pt"
        ...
        # Track the best validation r^2 score
        if r_value_va**2 > best_val_r2:
            print(f"better R2 valus just found as {r_value_va**2}")
            best_val_r2 = r_value_va**2
