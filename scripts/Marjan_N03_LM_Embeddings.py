...
#--------------------------------------------------#
### Note: install esm using this: pip install fair-esm
import esm
#====================================================================================================#
# Imports for MSA
...
class EmbeddingModel:
    def __init__(self, dataset_nme, models_list, model_select, data_folder, input_seqs_fasta_file, output_file_name_header, pretraining_name=None, batch_size=100, xlnet_mem_len=512):
        self.dataset_nme = dataset_nme
        self.models_list = models_list
        self.model_select = model_select
        self.data_folder = Path(data_folder)
        self.input_seqs_fasta_file = input_seqs_fasta_file
        self.output_file_name_header = output_file_name_header
        self.pretraining_name = pretraining_name
        self.batch_size = batch_size
        self.xlnet_mem_len = xlnet_mem_len
    def N03_embedding_LM(self, dataset_nme,
                         models_list,
                         model_select, 
                         data_folder ,
        ...
        if model_select == "ESM_1B":
            ...
            output_file_esm = data_folder / (output_file_name_header + model_select) #output path (pickle)
            ...
            chunk_size = 5000  # Number of sequences per chunk
            ...
            for i in range(0, len(data_set), batch_size):
                ...
                if len(seq_ids_chunk) >= chunk_size or (i + batch_size) >= len(data_set):
                    output_file_chunk = f"{output_file_esm}_{batch_counter}.p"
            ...
            # # Marjan: Initialize variables
            # chunk_size = 5000  # Number of sequences per file
            # batch_counter = 0  # File counter
            # seq_ids = []  # To store all sequence IDs
            ...
            #     # Save the embeddings in chunks
            #     #if (i // batch_size) % (chunk_size // batch_size) == 0 or (i + batch_size) >= len(data_set):
            #     if len(seq_ids) >= chunk_size or (i + batch_size) >= len(data_set):
            #         print( 90 * "*")
            #         output_file_chunk = f"{output_file_esm}_{batch_counter}.p"
            #         seq_embedding_output = {
            #             "seq_embeddings": np.stack(sequence_representations),
            #             "seq_ids": seq_ids,
